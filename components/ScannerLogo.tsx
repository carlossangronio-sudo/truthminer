...
/**
 * Logo Truth Scanner avec style cyber/neural
 */
export const ScannerLogo = ({ onClick, className = '' }: ScannerLogoProps) => {
  const content = (
    <div
      ...
      aria-label="Truth Scanner - Accueil"
    >
      ...
      <span className="text-xl font-black uppercase tracking-tighter italic text-white">
        Truth <span className="text-cyan-400">Scanner</span>
      </span>
    </div>
  );
  ...
  return (
    <Link href="/" aria-label="Truth Scanner - Accueil">
      {content}
    </Link>
  );
};
